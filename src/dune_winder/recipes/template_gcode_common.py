...
def split_queue_merge_prefix(line):
  parts = str(line).split(None, 2)
  if len(parts) < 2 or parts[0] != "G113" or not parts[1].startswith("P"):
...
def annotate_wrap_lines(wrap_number, lines, *, line_builder):
  ...
  for line_number, line in enumerate(lines, start=1):
    identifier = wrap_identifier(wrap_number, line_number)
    queue_merge_prefix, remainder = split_queue_merge_prefix(line)
    if queue_merge_prefix is None:
      annotated.append(line_builder(identifier, line))
      continue
    annotated.append(line_builder(queue_merge_prefix, identifier, remainder))
  return annotated

...
class PlcBranchSimplifierTests(unittest.TestCase):
  ...
  def test_lowers_ote_suffix_through_helper_bit(self):
    result = simplify_text(
    ...
    self.assertTrue(result.changed)
    self.assertEqual(result.issues, ())
    self.assertEqual(result.helper_tags, ("auto_main_branch_001",))
    self.assertEqual(
      self.emitter.emit_routine(result.routine),
      (
        "OTU auto_main_branch_001 \n"
        "XIC A OTL auto_main_branch_001 \n"
        "XIC B OTL auto_main_branch_001 \n"
        "XIC auto_main_branch_001 OTE output_bit \n"
      ),
    )
  ...
  def test_lowers_read_write_overlap_through_helper_bit(self):
    result = simplify_text(
    ...
    self.assertEqual(
      ...
      (
        "OTU auto_main_branch_001 \n"
        "XIC trigger XIC A OTL auto_main_branch_001 \n"
        "XIC trigger XIC B OTL auto_main_branch_001 \n"
        "XIC auto_main_branch_001 CPT count count+1 \n"
      ),
    )

  def test_lowers_motion_suffix_through_helper_bit(self):
    result = simplify_text(
      (
        "BST XIO TENSION_CONTROL_OK NXB XIO speed_regulator_switch BND "
        "XIC trigger_xy_move MCLM X_Y main_xy_move 0 X_POSITION XY_SPEED_REQ "
        "\"Units per sec\" XY_ACCELERATION \"Units per sec2\" XY_DECELERATION "
        "\"Units per sec2\" S-Curve 500 500 \"Units per sec3\" 0 Disabled "
        "Programmed 50 0 None 0 0 \n"
      ),
      routine_name="main",
    )
    ...
    self.assertEqual(
      ...
      (
        "OTU auto_main_branch_001 \n"
        "XIO TENSION_CONTROL_OK OTL auto_main_branch_001 \n"
        "XIO speed_regulator_switch OTL auto_main_branch_001 \n"
        "XIC auto_main_branch_001 XIC trigger_xy_move MCLM X_Y main_xy_move 0 "
        "X_POSITION XY_SPEED_REQ \"Units per sec\" XY_ACCELERATION "
        "\"Units per sec2\" XY_DECELERATION \"Units per sec2\" S-Curve 500 500 "
        "\"Units per sec3\" 0 Disabled Programmed 50 0 None 0 0 \n"
      ),
    )
  ...
  def test_simplify_file_rewrites_only_when_requested(self):
    with tempfile.TemporaryDirectory() as tmp_dir:
      ...
      self.assertEqual(dry_run_report.helper_tags, ())
      ...
      self.assertEqual(apply_report.helper_tags, ())

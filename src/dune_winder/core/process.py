...
class Process:
  ...
  def executeG_CodeLine(self, line: str):
    ...
    if not self.controlStateMachine.isReadyForMovement():
      ...
      xyf = r"(\ *[X]\d{1,4}(\.\d{1,2})?\ *[Y]\d{1,4}(\.\d{1,2})?\ *[F]\d{1,3}\ *$)"  # 'X1234 Y1234 F123'
      fxy = r"(\ *[F]\d{1,3}\ *[X]\d{1,4}(\.\d{1,2})?\ *[Y]\d{1,4}(\.\d{1,2})?\ *$)"  # 'X1234 Y1234 F123'
      xf = r"(\ *[X]\d{1,4}(\.\d{1,2})?\ *[F]\d{1,3}\ *$)"  # 'X1234 F123'
      fx = r"(\ *[F]\d{1,3}\ *[X]\d{1,4}(\.\d{1,2})?\ *$)"  # 'F123 X1234'
      yf = r"(\ *[Y]\d{1,4}(\.\d{1,2})?\ *[F]\d{1,3}\ *$)"  # 'Y1234 F123'
      fy = r"(\ *[F]\d{1,3}\ *[Y]\d{1,4}(\.\d{1,2})?\ *$)"  # 'F123 Y1234'
      f_only = r"(\ *[F]\d{1,3}(\.\d{1,2})?\ *$)"  # 'F123', 'F123.45'
      gxyf = r"(\ *[G]105\ *[P][XY]-?\d{1,3}(\.\d{1,2})?\ *[F]\d{1,3}\ *$)"  # 'G105 PX123 F12','G105 PY123 F123'
      gx_yf = r"(\ *[G]105\ *[P][X]-?\d{1,3}(\.\d{1,2})?\ *[P][Y]-?\d{1,3}(\.\d{1,2})?\ *[F]\d{1,3}\ *$)"  # 'G105  PX123 PY123 F123'

...
from dune_winder.library.XGTemplateGCode import WIRE_SPACING as GX_WIRE_SPACING
...
class ManualCalibrationTests(unittest.TestCase):
  ...
  def test_xg_generate_writes_live_gcode_and_refreshes_active_recipe(self):
    with tempfile.TemporaryDirectory() as rootDirectory:
      ...
      self.assertEqual(lines[1], "N0 X635.0 Y196.0\n")
      self.assertEqual(lines[2], "N1 (1,1) X635.0 Y196.0\n")
      self.assertEqual(lines[3], "N2 (1,2 HEAD RESTART) X440.0\n")
      self.assertEqual(lines[4], "N3 (1,3) X635.0\n")
      self.assertEqual(lines[5], "N4 (1,4) X7165.0 Y398.0\n")
      self.assertEqual(lines[-1], "N4321 X635.0 Y2496.0\n")
